import selectedWork from '../../img/work/wedding/wedding-hero-1.png';
import selectedWork0 from '../../img/work/choco/hero1.jpg';
import selectedWork1 from '../../img/work/xendo/menu/trifold-mockup-2.png';
import selectedWork2 from '../../img/work/qfg/hero.png';
import selectedWork3 from '../../img/work/xendo/events/hero-planning.png';
import selectedWork4 from '../../img/work/trixxo/trixxo-20-years.png';
import selectedWork5 from '../../img/work/xendo/social/xendo-social-media-facebook.png';
import selectedWork6 from '../../img/work/cabincrew/finnair-thankyou-min.png';
import selectedWork7 from '../../img/work/birthday/birthday-card-design-1.png';
import selectedWork8 from '../../img/work/tourette/tourette-webdesign-1.png';

const workItemsData = [
{
  id: 9,
  slug: "cabin-crew-thanks",
  image: selectedWork6,
  title: "Bedankkaartjes voor cabin crew",
  link: "/work/cabin-crew-thanks",
  description: "Vliegen draait niet alleen om de bestemming, maar ook om de ervaring aan boord: gecreëerd door de vriendelijkheid en inzet van het personeel. Als klein gebaar van waardering ben ik begonnen met het ontwerpen van bedankkaartjes voor het personeel, elke keer dat ik reis. Elk kaartje sluit perfect aan bij de huisstijl en uitstraling van de betreffende luchtvaartmaatschappij.",
  date: "2025-02-19",
  alt:
    {
      image_1: "Het bedankkaartje ontworpen voor AirAsia. Een vrouwelijke stewardess van AirAsia maakt een hartvorm met haar handen voor een A320-vliegtuig van AirAsia.",
      gallery_image_1: "Het bedankkaartje ontworpen voor Thai Airways. Een paars-roze kleurverloop onderaan met een vliegtuig van Thai Airways dat door een heldere blauwe lucht vliegt.",
      gallery_image_2: "Het bedankkaartje ontworpen voor Finnair. Een collage met twee afbeeldingen: een vrouwelijke stewardess en de staartvin van een Finnair-vliegtuig.",
      gallery_image_3: "Het bedankkaartje ontworpen voor AirAsia. Een vrouwelijke stewardess van AirAsia maakt een hartvorm met haar handen voor een A320-vliegtuig van AirAsia."
    }
  },
  {
    id: 8,
    slug: "xendo-socials",
    image: selectedWork5,
    title: "Sociale media voor Club Xendo",
    link: "/work/xendo-socials",
    description: "In samenwerking met de eventplanner brainstormden we over creatieve ideeën om de bezoekerservaring naar een hoger niveau te tillen. Voor elk concept ontwierp ik visuals (onder andere: eventbanners, vierkante posts en storyformats voor Facebook en Instagram). Van DJ-aankondigingen en speciale acties tot ticketupdates en giveaways: alles kwam samen in een dynamische en samenhangende online aanwezigheid. Daarnaast schreef ik mee aan de social media posts en verzorgde ik de promotie achter de schermen door gerichte advertenties op te zetten en te optimaliseren. Sinds januari 2025 beheer ik de social media en visuals van Club Xendo niet meer, aangezien zij dit nu intern opnemen.",
    date: "2025-01-27",
    alt: {
      image_1: "Een laptopscherm met de Facebookpagina van Club Xendo open in de browser, met bovenaan een post over het 90's event.",
      gallery_image_1: "Facebookpost over het Halloween-event met de aankondiging van de laatste early bird-tickets.",
      gallery_image_2: "Facebookpost met de aankondiging van alle dj’s voor het 90's event bij Club Xendo.",
      gallery_image_3: "Facebookpost met de aankondiging van alle dj’s voor het Halloween-event bij Club Xendo.",
      gallery_image_4: "Facebookbanner tijdens de zomerafsluiting.",
      gallery_image_5: "Facebook eventbanner voor 'Groovy Tunes Soundsystem invites...'.",
      gallery_image_6: "Facebook eventbanner voor het zaterdagse clubbing-event."
    }
  },
  {
    id: 7,
    image: selectedWork4,
    slug: "trixxo-20-years",
    title: "20 jaar TRIXXO",
    link: "/work/trixxo-20-years",
    description: "Wat is een betere manier om 20 jaar TRIXXO te vieren dan met een frisse en feestelijke merkuitbreiding? Ik ontwierp een logo-variant en campagne-idee dat de reis van TRIXXO eert, waarbij de kernwaarden worden gecombineerd met een gevoel van groei, verbinding en enthousiasme.",
    date: "2025-01-20",
    start_content: "Voor het 20-jarig bestaan van TRIXXO ontwierp ik een logovariant die trouw blijft aan de merkidentiteit, maar met een feestelijke touch. Het vuurwerksymbool staat voor het jubileumfeest, terwijl de iconen in het ontwerp de kernwaarden van het bedrijf weerspiegelen.",
    middle_content: "Naast het logo bedacht ik een interactieve teamcampagne: de TRIXXO Bingo. Werknemers worden uitgedaagd om leuke opdrachten uit te voeren, hun resultaten te delen op social media en op een creatieve manier met collega’s in interactie te gaan. Om de campagne te ondersteunen, ontwierp ik story- en vierkante formaten voor social media.",
    end_content: "Dit project was een goede oefening in het vinden van de juiste balans tussen merkconsistentie en een frisse, boeiende campagne die zowel de interne betrokkenheid als de zichtbaarheid van het merk vergroot.",
    alt: {
      image_1: "Een visitekaartje vasthouden met het nieuw ontworpen jubileumlogo voor 20 jaar TRIXXO",
      gallery_image_1: "Het story-formaat dat ik heb ontworpen voor de TRIXXO 20 jaar campagne",
      gallery_image_2: "Het vierkante formaat dat ik heb ontworpen voor de TRIXXO 20 jaar campagne"
    }
  },
  {
    id: 6,
    slug: "xendo-planning",
    image: selectedWork3,
    title: "Xendo Maandelijkse Planning",
    link: "/work/xendo-planning",
    description: "Elke maand worden er visuals voor de eventplanning van Club Xendo ontworpen. Deze geven een helder overzicht van alle aankomende evenementen. De visuals worden gecreëerd in verschillende formaten, waaronder een flyer, poster en social media stories. Het doel is om een opvallende en gestructureerde lay-out te bieden die meteen de aandacht trekt en bezoekers informeert over alle geplande evenementen. Elk ontwerp sluit aan bij de branding van Club Xendo, waardoor er een consistente visuele identiteit wordt gewaarborgd over alle formaten heen.",
    date: "2025-01-18",
    alt: {
      image_1: "Een flyer vasthouden met de planning van Xendo in oktober",
      gallery_image_1: "Drie grote posters ontworpen voor Club Xendo, van links naar rechts: Nieuwjaar, Ronny Retro en Moderna Printing team event",
      gallery_image_2: "Grote poster met de evenementen in oktober, gemaakt voor Club Xendo",
      gallery_image_3: "Foto van een vrouw die haar smartphone vasthoudt met het story format van de planning voor september",
      gallery_image_4: "Grote poster met de evenementen in december, gemaakt voor Club Xendo"
    }
  },
  {
    id: 5,
    slug: "quality-fence-gardens",
    image: selectedWork2,
    title: "Quality Fence & Gardens Logo",
    link: "/work/quality-fence-gardens",
    description: "Voor het logo van Quality Fence & Gardens heb ik verschillende stijlen verkend, van modern tot retro, zodat de klant zelf de gewenste richting kon bepalen. Het uiteindelijke ontwerp weerspiegelt de identiteit van hun merk met een krachtig en tijdloos design. Het logo wordt momenteel gebruikt op hun voertuigen en visitekaartjes.",
    date: "2025-01-12",
    alt: {
      image_1: "Quality Fence & Gardens-logo op hun voertuigen",
      gallery_image_1: "Definitieve versie van het Quality Fence & Gardens-logo",
    }
  },
  {
    id: 4,
    slug: "xendo-bar-menu",
    image: selectedWork1,
    title: "Drankkaart voor Club Xendo",
    link: "/work/xendo-bar-menu",
    date: "2025-01-05",
    description: "Het ontwerp van de drankkaart voor Club Xendo moest zowel visueel aantrekkelijk als overzichtelijk zijn, met een duidelijke weergave van de prijzen. De grootste uitdaging was het presenteren van de champagneselectie, waarbij flessen in verschillende formaten en prijsklassen van diverse merken moesten worden weergegeven. Het doel was een ontwerp te creëren dat de complexe prijsstructuur begrijpelijk maakte, terwijl het geheel stijlvol en modern uitstraalde.",
    alt: {
      image_1: "Groep vrouwen in Club Xendo die de drankkaart voor zich houden.",
      gallery_image_1: "Foto van de voor- en achterkant van de drankkaart van Club Xendo",
      gallery_image_2: "Foto van de voorkant van de drankkaart van Club Xendo",
      gallery_image_3: "Foto van de achterkant van de drankkaart van Club Xendo"
    }
  },
  {
    id: 3,
    slug: "wedding-invitation",
    image: selectedWork,
    title: "Trouwuitnodiging",
    link: "/work/wedding-invitation",
    description: "Voor deze trouwuitnodiging ontwierp ik een design dat perfect aansloot bij de visie van het koppel: een verfijnd, maar elegant bloemenmotief met een minimalistische en stijlvolle uitstraling. Het eindresultaat is een perfecte balans tussen romantiek en moderniteit, een uitnodiging die niet alleen informeert, maar ook de essentie van hun speciale dag weerspiegelt.",
    date: "2024-12-12",
    alt: {
      image_1: "Mock-up van het definitieve ontwerp voor de trouwuitnodiging",
      gallery_image_1: "De eerste logovariant met de initialen van het toekomstige bruidspaar, omringd door een bloemenkrans",
      gallery_image_2: "De tweede logovariant met de initialen van het toekomstige bruidspaar, omringd door een bloemenkrans",
      gallery_image_3: "Mock-up van het definitieve ontwerp voor de trouwuitnodiging"
    }
  },
  {
    id: 2,
    slug: "choco-logo",
    image: selectedWork0,
    title: "CHOCO Relatiebouwers afscheidscadeau",
    link: "/work/choco-logo",
    description: "Wat is een betere manier om een stage af te sluiten dan met een handgemaakt eerbetoon? Ik wilde iets betekenisvols creëren: een creatief stuk dat écht aansloot bij de identiteit van CHOCO Relatiebouwers.",
    start_content: "Ik maakte een 3D-versie van het logo, volledig opgebouwd uit lucifers. Elke luciferkop werd blauw geverfd om aan te sluiten bij de huisstijl van het bedrijf op dat moment. Het eindresultaat werd ingekaderd, waardoor het een uniek decoratief element werd voor hun kantoor.",
    end_content: "Daarnaast vulde ik hun stageboek met origami, handgetekende schetsen, een klein boekje met herinneringen en heel wat foto's. Een klein, maar waardevol gebaar om mijn dankbaarheid te tonen en een creatieve indruk achter te laten.",
    date: "2024-11-04",
    alt: {
      image_1: "Het afgewerkte CHOCO-logo in blauw geverfde lucifers, nog niet ingekaderd.",
      gallery_image_1: "Work-in-progress foto: de schets van het logo met al geverfde blauwe lucifers voor de stippen",
      gallery_image_2: "Work-in-progress foto: close-up van een stip gemaakt van geverfde blauwe lucifers",
      gallery_image_3: "Work-in-progress: de vorm van een van de 'O's' en hoe dit is opgebouwd. Eerst een silhouet gecreëerd en dit vervolgens gevuld met blauwe lucifers",
      gallery_image_4: "Linkerpagina van het stageboek: een collage van foto's van mijn stage bij CHOCO Relatiebouwers",
      gallery_image_5: "Rechterpagina van het stageboek: de vleugels die ik tekende, origami die ik maakte en het kleine boekje dat ik schreef met herinneringen aan de stage"
    }
  },
  {
    id: 1,
    slug: "tourette-webdesign",
    image: selectedWork8,
    title: "Website voor Vlaamse Vereniging Tourette",
    link: "/work/tourette-webdesign",
    description: "Tijdens mijn stage bij Content Cowboys kreeg ik de kans om een volledig nieuw webdesign te maken voor de Vlaamse Vereniging Gilles de la Tourette. De focus lag op duidelijke informatievoorziening en een warme, toegankelijke uitstraling.",
    date: "2024-09-01",
    start_content: "Tijdens een boeiende kick-offmeeting met de Vlaamse Tourettevereniging leerde ik hun visie kennen. Ze wilden een warme, toegankelijke website zonder foto's maar wel met illustraties: niet te cartoonachtig, maar realistisch en respectvol. Ondanks enkele beperkingen zoals een verouderde huisstijl en logo, ging ik creatief aan de slag om hun verhaal visueel sterker te brengen.",
    alt: {
      image_1: "Homepage van de Vlaamse Vereniging Gilles de la Tourette met een quote en illustratie van een kind",
      gallery_image_1: "Design moodboard als inspiratie voor de branding van de nieuwe huisstijl",
      gallery_image_2: "Overzicht van de pagina met uitleg over de symptomen van het syndroom van Gilles de la Tourette"
    }
  },
  {
    id: 0,
    slug: "birthday-card-design",
    image: selectedWork7,
    title: "Happy birthday kaart voor cupcake lover",
    link: "/work/birthday-card-design",
    description: "Deze kaart ontwierp ik voor iemand die niet gewoon fan is van cupcakes: ze ademt letterlijk cupcakes. Als ze er niet aan het maken is, is ze waarschijnlijk wel bezig met nieuwe topping ideeën of allerlei decoratie te verzamelen. Het kaartje is een speelse knipoog naar haar grootste passie.",
    date: "2024-08-12",
    alt: {
      image_1: "Ontwerp van een verjaardagskaart met cupcakes en typografie in muntgroene tinten",
    }
  }
];

export default workItemsData;